...
function Register() {
  ...
  const submit = (e) =>{
    ...
    axios.post("https://backend-ashen-gamma.vercel.app/register", userData).then((res)=>{
      console.log(res.data)

    axios.post(`https://backend-ashen-gamma.vercel.app/send-otp`, {email: userData?.email}).then((res)=>{
      console.log(res.data)
  }).catch((err)=>{
      console.log(err)
  })
  ...
  return (
      ...
      <div color="transparent" className="flex items-center justify-center" >
        ...
        <form className="mt-8 mb-2 w-80 max-w-screen-lg sm:w-96 bg-light px-10 py-10 rounded-lg shadow-2xl">
          ...
          <a href="https://backend-ashen-gamma.vercel.app/auth"><Button className="mt-6 bg-light rounded-full bg-gray-500" fullWidth>
            Login with google
          </Button></a>
